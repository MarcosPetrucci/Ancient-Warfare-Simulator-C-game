...
//!Essa constante define o valor da velocidade dos personagens e dos proj�teis
//!Caso necess�rio, altere esse valor para calibrar a velocidade
#define VELOCIDADE 0.07 //recomendamos aumentar de 0.1 em 0.1 at� encontrar o valor ideal

//!Essa constante define as propor��es da tela
...
void configurar()
{
    ...
    SDL_GL_SetAttribute(SDL_GL_DEPTH_SIZE, 16); //3D - n�o sei se vamos usar
    SDL_GL_SetAttribute(SDL_GL_DOUBLEBUFFER, 1); //3D - n�o sei se vamos usar
    ...
    //2D (COOL)
    glMatrixMode(GL_PROJECTION); //Aplica a proje��o da matriz atual
    glLoadIdentity();
...
{
    //Detectar colis�o em todos os lados
...
void carregaDados_Jogador_Conexao() //Carrega informa��es dos dois jogadores ativos!
{
    //Considerando que os jogadores ja digitaram seus nomes e escolheram seus cadastros
    //Ou se eles j� tiverem jogado, considerando que seus registros j� foram devidamente salvos no arquivo conex�o
    //Essa fun��o le este arquivo conexao e carrega os unicos dois registros gravados neste nos struct usuario Jogador1 e Jogador2

    abrir_conexao("ab+"); //Neste programa, meu unico interesse � ler o arquivo
    arquivo = true;
...
void salvaDados_Jogadores_Conexao() //Salva no arquivo as inforam��es de um NOVO Jogador
{
    abrir_conexao("wb+");
...
{
    //Fun��o que escreve os respectivos cora��es para os dois jogadores
    int i;
    float x, y = 570;
...
void movimento(struct movimento *J1, struct movimento *J2, float *J1X, float *J1Y, float *J2X, float *J2Y,
                ...
                float *c1X, float *c1Y, float *c2X, float *c2Y, float cLado) //Processa todos os movimentos necess�rios, e retorna os valores das coordernadas
{
    int i;
    ...
    if(J1->shot)
    {
        ...
        for(i = 0; i < 100 ; i++)
        {
            ...
            {
                tiros1[i][0] =  *J1X + J1Comp; //O tiro come�a na mesma posi��o do personagem
    ...
    if(J2->shot)
    {
        ...
        for(i = 0; i < 100; i++)
        {
            if(!tiros2[i][2])
            {
                ...
                tiros2[i][2] = 1.0; //indica que h� esse tiro
                i = 100;
            }
        }
    }
    //Verificar se devo gerar cora��es
    time_t tempo = time(NULL);
    if((double) (tempo - t_coracao) >= 10) //Deve enviar o comando para gerar um cora��o em posi��o aleatoria a cada 12.5s
    {
        t_coracao = tempo;
        *coracao1 = true;
        *coracao2 = true;

        *c1X = rand()%400 - cLado; //!Adicionar aqui os tamanhos corretos dos cora��es!
        *c1Y = rand()%550 - cLado;
...
int main(int argc, char* args[])
{
    ...
    float **tiros1; //A primeira coluna � a posi��o X, a segunda Y, a terceira indica se aquele tiro ainda existe
    float **tiros2; //A primeira coluna � a posi��o X, a segunda Y, a terceira indica se aquele tiro ainda existe
    ...
    bool coracao1 = false; //Variavel que indica se um cora��o deve ser gerado em local aleat�rio
    bool coracao2 = false;
    ...
    float c1X = 900; //Vari�veis que armazenas as coordenadas X e Y dos possiveis cora��es gerados;
    float c1Y = 500; //Come�am com esses valores para que eles come�em fora da tela
    float c2X = 900;
    float c2Y = 500;
    float cLado = 25; //Cora��o quadrado


    //Vari�veis para movimentar o personagem
    struct movimento J1;
    struct movimento J2;
    ...
    ///Fun��o deve detectar a imagem corretamente, dado os ids dos personagens
    ...
    Mix_PlayMusic(musica, -1); //Inicia a m�sica do menu
    ...
    while(executando)
    {
        ...
        while(SDL_PollEvent(&eventos))
        {
            ...
            { //enquanto a tecla esta pressionada, movimentar
                ...
                {
                    tempo = time(NULL);
                    if(comecou_jogo && (double) (tempo - Tick[0]) >= 1.5) //A diferen�a entre o tiro anterior e esse tem que ser maior que 1s
                    {
                        Tick[0] = tempo;
                        J1.shot = true;
                    }
                ...
                {
                    tempo = time(NULL);
                    if(comecou_jogo && (double) (tempo - Tick[1]) >= 1.5) //A diferen�a entre o tiro anterior e esse tem que ser maior que 1s
                    {
                        Tick[1] = tempo;
                        J2.shot = true;
                    }
                ...
                { //Botao jogar
                    if(!creditos && !comecou_jogo && !instrucoes) //Garante que est�  no menu, ou seja, nenhuma das  outras telas est� ativa
                    {
                        Mix_HaltMusic();
                        Mix_PlayMusic(musica1, -1);
                        t_coracao = time(NULL); //Inicia o contador dos cora��oes corretamente, ap�s o in�cio do jogo
                        comecou_jogo = true;
                    }
        ...
        else if(creditos) //Ativa a tela de cr�ditos
        {
                tempo = time(NULL);

                if((double) tempo - inicio < 15)
                {
                    //Primeiro cr�ditos aos artistas
        ...
        else if(instrucoes) //Ativa a tela de instru��es
        {
                tempo = time(NULL);
        ...
        else if(comecou_jogo) //comecou_jogo, ativa o jogo
        {
            ...
            if(!acabou_jogo)
            {
                ...
                movimento(&J1, &J2, &J1X, &J1Y, &J2X, &J2Y, tiros1, tiros2, J1Comp, J1Alt, J2Comp, J2Alt, &coracao1, &coracao2, &c1X, &c1Y, &c2X, &c2Y, cLado); //Processa todo o movimento necess�rio
                ...
                { //Aqui os projetos ser�o desenhados, e as colis�es ser�o detectadas

                    //Atualiza as posi��es dos tiros
                    if(tiros1[i][2] != 0.0) // Coordenadas do tiro s�o (tiro1[i][0] ; tiros1[i][1])
                    {
                        glEnable(GL_TEXTURE_2D);
                        ...
                        //Desenha - atualiza a posi��o dos tiros

                        //Ve se teve colis�o
                        if(colisao(J2X, J2Y, J2Comp, J2Alt, tiros1[i][0], tiros1[i][1], TiroComp, TiroAlt)) //Ocorreu colis�o
                        {
                            //Colis�o com o Jogador2
                            ...
                            if(!J2.vidas) //Se n�o tem mais vidas, ou seja morreu
                            {
                                //Acaba o jogo
                                Jogador1.vitorias++;
                                Jogador2.derrotas++;
                        ...
                        {   //Caso n�o tenha ocorrido colis�o, o tiro deve continuar seu trajeto
                            glColor4ub(255,255,255,255);
                            ...
                            tiros1[i][0] += VELOCIDADE * 4; //Atualiza a posi��o do tiro
                        }
                        glEnd();
                        glDisable(GL_TEXTURE_2D);
                    }
                    if(tiros2[i][2] != 0.0)
                    {
                        ...
                        if(tiros2[i][0] < 0)
                            tiros2[i][2] = 0.0;
                        //Detectar colis�o com os personagens, projetil por projetil!!!!

                        //Ve se teve colis�o // Some com o tiro, computa o dano
                ...
                //Detecta se  houve colis�o com o cora��o, ou seja, se os jogadores pegaram seus cora��es extras;

                if(colisao(J1X, J1Y, J1Comp, J1Alt, c1X, c1Y, cLado, cLado) && coracao1 && J1.vidas < 5) //NOVAMENTE, AQUI ARRUMAR OS COMPRIMENTOS E ALTURAS DOS CORA��ES
                { //O fato de o cora��o n�o estar desenhado n�o impede a fun��o colisao de retornar true
                    J1.vidas++;
                    coracao1 = false;
                }
                if(colisao(J2X, J2Y, J2Comp, J2Alt, c2X, c2Y, cLado, cLado) && coracao2 && J2.vidas < 5) //NOVAMENTE, AQUI ARRUMAR OS COMPRIMENTOS E ALTURAS DOS CORA��ES
                { //Aqui, se eu n�o testar se coracao2 � falso, mesmo que o cora��o suma, quando o jogador "Colide" com as coordenadas dele, ele vai ganhar uma vida
            ...
            {
                tempo = time(NULL);
                if(musica_toca == 1) //Troca a musica, e a mant�m at� o final
                {
                    ...
                    salvaDados_Jogadores_Conexao(); //Salva no arquivo ponte as informa��es recentes
                    //Isso � bom pois possibilita revanche sem ter que passar pelo menu
                    Mix_HaltMusic();
                    Mix_VolumeMusic(MIX_MAX_VOLUME/2);
        ...
        //Anima��o -- precisa definir mas � meio inutil
        SDL_GL_SwapBuffers();
